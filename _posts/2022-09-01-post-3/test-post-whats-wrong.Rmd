...
In fact, there's even an R package available for it thanks to [Chris Dalzell and his team](https://github.com/cdalzell/Lahman).

Let's use this dataset to figure out if homerun rates are high now compared to historical data.
First, what's the average rate of home runs per plate appearance over time? Checking the data...
...
```
It looks like 2020 was an outlier, so let's smooth the data and look for a trend over time 
by checking the time series is stationary. We can use the Augmented Dickey-Fuller test
from the tseries package.
```{r}
hr_rate_over_time_smooth <- hr_rate_over_time+
  geom_smooth()

hr_rate_over_time_smooth_plotly <- ggplotly(hr_rate_over_time_smooth)
hr_rate_over_time_plotly
